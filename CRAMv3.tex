...
the `prefix'. The number of leading 1-bits in the prefix denotes the number of
bytes to follow. Unlike UTF-8, only the first byte carries a prefix;
all subsequent bytes contain pure data bits with no continuation markers.

The bit patterns for each encoding length are as follows:

\begin{tabular}{|c|c|c|}
\hline
\textbf{Bytes} & \textbf{Byte 1} & \textbf{Data bits}\tabularnewline
\hline
1 & \texttt{0xxxxxxx} & 7\tabularnewline
\hline
2 & \texttt{10xxxxxx} & 6 + 8 = 14\tabularnewline
\hline
3 & \texttt{110xxxxx} & 5 + 16 = 21\tabularnewline
\hline
4 & \texttt{1110xxxx} & 4 + 24 = 28\tabularnewline
\hline
5 & \texttt{1111xxxx} & 4 + 24 = 28\tabularnewline
\hline
\end{tabular}

\noindent Note that in the 5-byte case, the prefix is \texttt{1111} with no trailing 0-bit,
and only the 4 lower bits of the first byte are used for data. This limits the
effective range to 32 bits (matching a signed 32-bit integer).

\item[LTF-8 long (ltf8)]\ \newline
See ITF-8 for more details. The only difference between ITF-8 and LTF-8 is the
number of bytes used to encode a single value. To encode 64 bits, up to
9 bytes are required. The bit patterns for each encoding length follow the same
scheme as ITF-8, extended through 8 leading 1-bits. In the 9-byte case, the
first byte is \texttt{0xFF} (all 1-bits, no trailing 0-bit), and the remaining
8 bytes contain 64 data bits.
